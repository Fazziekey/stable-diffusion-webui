...
from modules.extras_thread import ExtrasThread
...
def run_postprocessing(extras_mode, image, image_folder, input_dir, output_dir, show_extras_results, *args, save_output: bool = True):
    ...
    if cmd_opts.extra_multithread:
        upscale_thread = {}
        pp_result = {}
        for image, name in zip(image_data, image_names):
            upscale_thread[name] = ExtrasThread(name, image, args)
            upscale_thread[name].start()
        for name, thread in upscale_thread.items():
            pp_result[name] = thread.join()
