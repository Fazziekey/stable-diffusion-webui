...
from fast_models import CLIP, UNet, VAE
import numpy as np
import nvtx
import os
import onnx
...
from fast_utilities import Engine, DPMScheduler, LMSDiscreteScheduler, save_image, TRT_LOGGER

def parseArgs():
    parser = argparse.ArgumentParser(description="Options for Fast Diffusion")
...
if __name__ == "__main__":
    ...
    print("[I] Initializing Fast StableDiffusion with TensorRT Plugins")
    args = parseArgs()
    ...
    print("[I] Initialize FastDiffusion ..")
    # Initialize FastDiffusion
    fast_diffusion = FastDiffusion(
    ...
    print("[I] Load TensorRT engines and pytorch modules..")
    # Load TensorRT engines and pytorch modules
    fast_diffusion.loadEngines(args.engine_dir, args.onnx_dir, args.onnx_opset, 
    ...
    fast_diffusion.loadModules()

    print("[I] Warming up ..")
    for _ in range(args.num_warmup_runs):
        images = fast_diffusion.infer(prompt, negative_prompt, image_height, image_width, warmup=True, verbose=False)

    print("[I] Running StableDiffusion pipeline")
    if args.nvtx_profile:
        cudart.cudaProfilerStart()
    images = fast_diffusion.infer(prompt, negative_prompt, image_height, image_width, verbose=args.verbose)
    if args.nvtx_profile:
        cudart.cudaProfilerStop()

    fast_diffusion.teardown()
